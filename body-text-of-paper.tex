...
Genetic samples were collected from 160 invasive wild pigs across a SPATIAL EXTENT in 
...
all two-population combinations of $F_1$, $F_2$, $B_1$ and $B_2$. We also 
...
SNP filtering resulted in 14,233 loci that were used for subsequent analysis. Using the 160 empirical samples, ADMIXTURE results supported the identification of 3 genetic clusters (SUPPLEMENTAL FIGURE CROSS VALIDATION?). Admixture *q* values clearly grouped individuals from each of the core populations into 3 clusters while the 40 individuals from the contact regions were highly admixed (SUPPLEMENTAL FIGURE ADMIXTURE). The
$F_{ST}$ divergence from ADMIXTURE was: $F_{ST} = 0.243$ for Pop1-Pop2, $F_{ST} = 0.145$ for Pop1-Pop14 and $F_{ST} = 0.133$ for Pop2-Pop14.
