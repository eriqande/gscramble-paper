...
attributes that magnify the effect of RISPI.
...
either CCT (cut-throat trout), $F_1$, $F_2$, $BC_1$--CCT, or $BC_2$--CCT, that the MAP estimate of their
...
Here, we present \gscramble{}, a simulation approach for creating biologically informed individual 
...
alleles sampled for an individual increases. Additionally, we demonstrate the importance of accounting for 
...
\gscramble{} is conducive for analysis of numerous types of marker sets available from contemporary data generation 
...
specified pedigrees and, therefore, requires genotypes to be resolved. 
...
the study organism could still benefit from using gscramble to simulate assumed linkage. To do 
so, users of gscramble would need to randomly distribution markers across, and within, the 
...
assuming independence (Figure 5). Characterizing uncertainty would provide caution for 
researchers to not over interpret conclusions drawn in the absence of a recombination map.
